function startGame() {
  ...
  var playsheet = SpreadsheetApp.getActive().getSheetByName('Play area');
  ...
  var templatesheet = SpreadsheetApp.getActive().getSheetByName('Template player');
  for(var t=0;t<numofplayers;t++){
    const copysheet = templatesheet.copyTo(SpreadsheetApp.getActive());
  ...
  sendWebhookMessage(textofmessage);
}

function button1(xinput) {
  var configsheet = SpreadsheetApp.getActive().getSheetByName('Config');
  var playsheet = SpreadsheetApp.getActive().getSheetByName('Play area');
  ...
  if (buttonfunction.toString() == "Submit card"){ // check if function is to start an auction
    ...
    if(!isEndRound(parseInt(auctioncard))){
      startAuction(parseInt(auctioncard),auctype);
    } else {endRound(parseInt(auctioncard))}
  ...
  if (buttonfunction.toString() == "Submit festpreis"){// check if function is to submit festpreis
    ...
    if (result.getSelectedButton() == ui.Button.OK) {
      ...
      setFestpreis(parseInt(festpreis));        
    }
    // var cardtoauctioninputcell = configsheet.getRange("B10").getValue(); // find card inputcel from config
  ...
  if(buttonfunction.toString().substr(-11,11) == "second card"){// check if function is to add second card for a double auction
    ...
    var result = ui.prompt('Enter card number:', ui.ButtonSet.OK_CANCEL);
    var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
    var cardsheet = SpreadsheetApp.getActive().getSheetByName('Card details');
    var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
    var auctioneer = statesheet.getRange(curstate,1).getValue().substr(-1,1);
...
function button2() {
  ...
  if(buttonfunction.toString() == "Pass on card"){//check if function is to pass festpreis card
    ...
    var seller = curstatecode.substring(1,2); // get seller code
    var price = statesheet.getRange(parseInt(curstate),(seller.charCodeAt(0)-65)*2+2).getValue().toString(); // get the festpreis
    passFestpreis(seller,passer,price);
  }
...
function button7() {
  SpreadsheetApp.getActive().getSheetByName('Game setup').getRange('F10').setValue('test success');
...
function startAuction(auctioncard,auctype) {
  var setupsheet = SpreadsheetApp.getActive().getSheetByName('Game setup');
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
  ...
  sendWebhookImage(auctioncard)

  
}

function findAuctionType(cardnum) {
  var sheet = SpreadsheetApp.getActive().getSheetByName('Card details');
...
function setFestpreis(festpreis){
  var setupsheet = SpreadsheetApp.getActive().getSheetByName('Game setup');
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
  var laststate = statesheet.getRange("A".concat(curstate,":CC",curstate)).getValues(); // pull the entirety of the current state
  ...
  sendWebhookMessage("**Festpreis set at ".concat(festpreis.toString(),".** ",setupsheet.getRange(4+(nextplayer.charCodeAt(0)-65),3).getValue().toString(),"'s turn to buy or pass."));
}
...
function buyCard(seller, buyer, price){
  var setupsheet = SpreadsheetApp.getActive().getSheetByName('Game setup');
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
  ...
  addAuctionLog(seller, buyer,price,auctionedcards);
  sendWebhookMessage("**".concat(setupsheet.getRange(4+(nextplayer.charCodeAt(0)-65),3).getValue().toString(),"'s turn** to pick card for auction."))
}

function passFestpreis(seller,passer,price){
  var setupsheet = SpreadsheetApp.getActive().getSheetByName('Game setup');
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var numofplayers = statesheet.getRange("CH1").getValue().toString(); // get the number of players
  if(getNextPlayer(passer,numofplayers) == seller){ // seller buys card themselves if everyone else has passed
    buyCard(seller,seller,price);
  } else {
    var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
    var laststate = statesheet.getRange("A".concat(curstate,":CC",curstate)).getValues(); // pull the entirety of the current state
    var nextstate = (parseInt(curstate)+1).toString();
  ...
  sendWebhookMessage("**".concat(setupsheet.getRange(4+(nextplayer.charCodeAt(0)-65),3).getValue().toString(),"'s turn** to buy or pass."))
}
...
function passSecondInDouble(){
  ...
  if(auctioneer == nextplayer){
    buyCard(auctioneer,auctioneer,0);
  } else {
    var laststate = statesheet.getRange("A".concat(curstate,":CC",curstate)).getValues(); // pull the entirety of the current state
    var nextstate = (parseInt(curstate)+1).toString();
    statesheet.getRange("A".concat(nextstate,":CC",nextstate)).setValues(laststate); // set the next state as identical to the current one
    statesheet.getRange("A".concat(nextstate)).setValue("4".concat(auctioneer,nextplayer)); // set the correct next state
    sendWebhookMessage("**".concat(setupsheet.getRange(4+(nextplayer.charCodeAt(0)-65),3).getValue().toString(),"'s turn** to choose a second card or pass."))
  }
}

function addBid(player){// create a popup for the player to add their bid
  var ui = SpreadsheetApp.getUi();
  ...
  if (result.getSelectedButton() == ui.Button.OK) {
    ...
    if(statesheet.getRange(parseInt(curstate),1).getValue().substring(0,1) == "6"){
      if(!isIntOrX(value,-1,player,false)){
        throw new Error("Invalid input: please bid a number between 0 and your cash.")
      }
      sendWebhookMessage("**".concat(setupsheet.getRange(4+(player.charCodeAt(0)-65),3).getValue().toString()," bids** in hidden auction."))
    }
    ...
    if(statesheet.getRange(parseInt(curstate),1).getValue().substring(0,1) == "5"){
      ...
      if(!isIntOrX(value,strictupperbound,player,true)){
        throw new Error('Invalid input: you need to bid between the highest current bid and your cash or pass with "X".')
      }
      sendWebhookMessage("**".concat(setupsheet.getRange(4+(player.charCodeAt(0)-65),3).getValue().toString()," bids ",value.toString(),"** in open auction."))
    }
    statesheet.getRange(parseInt(curstate),(player.charCodeAt(0)-65)*2+2).setValue(value); // set the bid
  }
}

function isIntOrX(value,strictlowerbound,bidder,includeX) {
...
function checkHiddenAuctionOver(){  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
  ...
  buyCard(auctioneer, String.fromCharCode(findMax(bids,auctioneer,numofplayers)+65),Math.max.apply(null,bids))
  }
}

function checkOpenAuctionOver(){
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
  var numofplayers = parseInt(statesheet.getRange("CH1").getValue().toString()); // get number of players
  var auctioneer = statesheet.getRange(parseInt(curstate),1).getValue().toString().substring(1,2);
  ...
  if(bidders.length == 1){
    buyCard(auctioneer,String.fromCharCode(bidders[0]+65),bids[0])
  }
}
...
function endRound(auctioncard){
  ...
  var nextplayer = getNextPlayer(auctioneer,numofplayers)
  var topthree = artistOrder(curstate,auctioncard);
  ...
  payOut(round,topthree,curstate,numofplayers);
  dealCards(round,numofplayers);
  discardOwnedCards();
  if(round.toString()=="4"){
    ...
    sendWebhookMessage("Game over!".concat(" ",result,"."));
  } else {
    ...
    sendWebhookMessage("New round started. **".concat(setupsheet.getRange(4+(nextplayer.charCodeAt(0)-65),3).getValue().toString(),"'s turn** to pick a card for auction."))
  }
}


function isEndRound(auctioncard){
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
...
function countBadingadoo(curstate){
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var cardcount=0;
  for(i=0;i<16;i++){
...
function artistOrder(curstate,auctioncard){
  var artisttotals = [countHoyos(curstate)+.5,countKrump(curstate)+.4,countConst(curstate)+.3,countWou(curstate)+.2,countBadingadoo(curstate)+.1];
  if(0<auctioncard && auctioncard<13){
    artisttotals[0] = 5;
  } else if(12<auctioncard && auctioncard<26){
    artisttotals[1] = 5;
...
function payOut(round,topthree,curstate,numofplayers){
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var playsheet = SpreadsheetApp.getActive().getSheetByName('Play area');
...
function submitOnceAroundBid(){
  ...
  if (result.getSelectedButton() == ui.Button.OK) {
    ...
    if(!isIntOrX(value,strictupperbound,bidder,true)){
    ...
    if(bidder == auctioneer){
      ...
      buyCard(auctioneer,String.fromCharCode(buyer+65),max);
    } else {
      statesheet.getRange("A".concat(nextstate,":CC",nextstate)).setValues(laststate); // set the next state as identical to the current one
      ...
      sendWebhookMessage("**".concat(setupsheet.getRange(4+(bidder.charCodeAt(0)-65),3).getValue().toString()," bids ",value.toString(),"** in a once-around auction. **",setupsheet.getRange(4+(getNextPlayer(bidder,numofplayers).charCodeAt(0)-65),3).getValue().toString(),"'s turn** to bid."))
    }

  }
}

function dealCards(round,numofplayers){
  var statesheet = SpreadsheetApp.getActive().getSheetByName('Game state');
  var curstate = statesheet.getRange("CF1").getValue().toString(); // find the position of the current state
  var deckcards = [];
  for(var t=0;t<70;t++){
...
function addAuctionLog(seller, buyer,price,auctionedcards){
  var playsheet = SpreadsheetApp.getActive().getSheetByName('Play area');
  var setupsheet = SpreadsheetApp.getActive().getSheetByName('Game setup');
...
function sendWebhookMessage(textofmessage) {
  var setupsheet = SpreadsheetApp.getActive().getSheetByName('Game setup');
...
function sendWebhookImage(imagenumber) {
  var curspreadsheet = SpreadsheetApp.getActive();
